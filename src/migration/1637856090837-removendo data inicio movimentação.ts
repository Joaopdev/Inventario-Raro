...
export class removendoDataInicioMovimentação1637856090837 implements MigrationInterface {
    name = 'removendoDataInicioMovimentação1637856090837'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`movimentacao\` DROP COLUMN \`dataInicio\``);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`endereco\` ADD CONSTRAINT \`FK_172b2625739dfaa7edac4c712ae\` FOREIGN KEY (\`colaboradorId\`) REFERENCES \`colaborador\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`movimentacao\` ADD \`dataInicio\` datetime NOT NULL`);
    }
