...
export class AdcionandoAtivoEmTipoEquipamentoEEquipamento1638055946020 implements MigrationInterface {
    name = 'AdcionandoAtivoEmTipoEquipamentoEEquipamento1638055946020'

    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`ALTER TABLE \`tipo_equipamento\` ADD \`ativo\` tinyint NOT NULL DEFAULT 1`);
        await queryRunner.query(`ALTER TABLE \`equipamento\` ADD \`ativo\` tinyint NOT NULL DEFAULT 1`);
        ...
        await queryRunner.query(`ALTER TABLE \`movimentacao\` ADD CONSTRAINT \`FK_9cb24bca1d212ecbd4427bf8010\` FOREIGN KEY (\`equipamentoId\`) REFERENCES \`equipamento\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`endereco\` ADD CONSTRAINT \`FK_172b2625739dfaa7edac4c712ae\` FOREIGN KEY (\`colaboradorId\`) REFERENCES \`colaborador\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`equipamento\` DROP COLUMN \`ativo\``);
        await queryRunner.query(`ALTER TABLE \`tipo_equipamento\` DROP COLUMN \`ativo\``);
    }
